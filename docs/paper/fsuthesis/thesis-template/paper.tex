...
Pin, developed by Intel Corporation and the University of Colorado, performs dynamic binary instrumentation on x86, Itanium, and ARM architectures. Pin takes an arbitrary binary to be instrumented and an user-developed dynamically linked library, and instruments the binary at runtime by inserted user defined routines or changes into the application's code.

Pin's architecture is two-fold: consisting of a virtual machine for compiling and executing code, and a cache for previously executed 

instrumentation is accomplished with Unix ptrace, similar to debuggers

TODO finish pin stuff
