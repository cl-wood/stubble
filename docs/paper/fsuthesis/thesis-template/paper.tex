...
The GUI explorer handles the event space by exploring all possible sequences of user-defined events at random. In order to discover event handlers, the GUI explorer relies on information from Kudzu, which instruments WebKit functions. When an HTML element which can generate an event is found, or when one of these elements in removed, Kudzu's hooks notice this and inform the GUI explorer accordingly.

[AD]
new things contributed
challenges - js strings, GUI elements
arch. use WebKit browser, 
js example of challenge
shortcomings - don't support all string ops, nor all JavaScript regex functionality, tradeoff
results - lots of found vulns involve strings, 
