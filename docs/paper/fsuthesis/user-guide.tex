...
packaged and distributed as a zip file.  When the zip file is
unpacked, a folder called \complit{fsuthesis} is created.  In that
folder will be found this \booktitle{User's Guide}, in both its
\acro{PDF} and \LaTeX{} source form, as well as a few other files, the
\complit{thesis-template} folder, and a \complit{sample} folder.

The \fsuth{} class file is called \complit{fsuthesis.cls}.  Using
...
go.  You can copy the \complit{thesis-template} directory to a new
...
\macroitems{semester}
This should be set to one of\index{semester@\verb+\semester+}
...
The date\index{defensedate@\verb+\defensedate+} of your thesis,
...
\verb|\label{eq:euler-id}| creates the label key
``\complit{eq:euler-id}'', tied to the automatically-numbered
equation.  To access the equation number, we use the
...
\begin{description}
\item[\pkg{geometry}, \pkg{spacing}] \emph{Do not use these packages
  ...
  interactions between settings in \pkg{geometry} or \pkg{spacing}
